...
![問題架構與方法層次圖](assets/figures/fig06-architecture.png)

*圖 1：系統三層架構——輸入層（地圖/乘客/端點）、優化層（5 種方法共用 A* 引擎）、評估層（成本/覆蓋率/統計）*
...
![總成本比較柱狀圖](assets/figures/fig01-total-cost.png)

*圖 2：五種方法在 5 張地圖上的總成本比較。GA/SA 在 Map 2-5 大幅優於 Baseline/Greedy。*
...
![覆蓋率比較柱狀圖](assets/figures/fig02-coverage.png)

*圖 3：覆蓋率比較（步行距離 ≤ 10 格）。GA/SA 透過路線偏移大幅提升乘客覆蓋。*
...
![GA vs SA 穩定性箱型圖](assets/figures/fig03-stability-boxplot.png)

*圖 4：GA vs SA 穩定性比較。SA 在所有地圖上的標準差均小於 GA，尤其 Map 3/4 差距明顯。*
...
![Map 2 路線形狀對比](assets/figures/fig05-route-comparison.png)

*圖 5：Baseline 直線路徑（覆蓋率 13%）vs SA 偏移路線（覆蓋率 100%），路線形狀是成本差異的關鍵。*
...
![消融實驗雷達圖](assets/figures/fig04-ablation-radar.png)

*圖 6：五組權重策略的三項成本雷達圖。不同策略在路線/步行/站牌成本間呈現清晰的取捨關係。*
