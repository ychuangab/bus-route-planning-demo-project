...
![Fig.6 — 問題架構與方法層次圖](assets/figures/fig06-architecture.png)

*Fig.6：系統三層架構——輸入層（地圖/乘客/端點）、優化層（5 種方法共用 A* 引擎）、評估層（成本/覆蓋率/統計）*
...
> **MILP 說明：** Map 1 成功求解（0.5s）。Map 2 於 60 秒超時後給出非最優解（*標記），總成本 2769 甚至高於 Baseline（1693），原因是 MILP 固定 A* 最短路徑、僅優化站牌位置，無法像 GA/SA 透過中繼點繞向乘客群聚區，導致站牌遠離乘客、步行成本極高。Map 3/5 在時限內未求解，Map 4 因規模過大（500 人）跳過。

![Fig.1 — 總成本比較柱狀圖](assets/figures/fig01-total-cost.png)

*Fig.1：五種方法在 5 張地圖上的總成本比較。GA/SA 在 Map 2-5 大幅優於 Baseline/Greedy。*
...
> *MILP Map 2 覆蓋率僅 7%，低於 Baseline（13%），因超時解將站牌集中於路線中段而非乘客群聚處。

![Fig.2 — 覆蓋率比較柱狀圖](assets/figures/fig02-coverage.png)

*Fig.2：覆蓋率比較（步行距離 ≤ 10 格）。GA/SA 透過路線偏移大幅提升乘客覆蓋。*
...
![Fig.3 — GA vs SA 穩定性箱型圖](assets/figures/fig03-stability-boxplot.png)

*Fig.3：GA vs SA 穩定性比較。SA 在所有地圖上的標準差均小於 GA，尤其 Map 3/4 差距明顯。*

### 6.5 執行時間比較

| 地圖 | Baseline | Greedy | GA | SA | MILP |
|------|----------|--------|-----|-----|------|
| Map 1 (20人) | <0.01s | <0.01s | 3.0s | 3.0s | 0.5s |
| Map 2 (100人) | <0.01s | 0.03s | 42.6s | 22.4s | 60.1s (超時) |
| Map 3 (100人) | <0.01s | 0.04s | 63.4s | 23.7s | 60.2s (超時) |
| Map 4 (500人) | 0.01s | 0.91s | 90.1s | 38.5s | — |
| Map 5 (100人) | 0.01s | 0.06s | 52.2s | 24.9s | 64.8s (超時) |

> Baseline/Greedy 為毫秒級確定性演算法。SA 在相同規模下比 GA 快約 40-60%，因為 SA 為單解搜索、不需維護族群。MILP 在 Map 2/3/5 均達 60 秒超時上限。

### 6.6 MILP 最優性分析（Map 1）

Map 1 是唯一 MILP 成功求解的地圖，可作為最優性基準。但需注意：**MILP 僅優化站牌位置（固定 A* 最短路線），而 GA/SA 同時優化路線形狀與站牌位置**，因此 GA/SA 的搜索空間更大，有機會找到 MILP 無法達到的解。

| 方法 | 總成本 | 與 MILP 差距 | 說明 |
|------|--------|-------------|------|
| MILP | 222.0 | — | 固定路線下的站牌最優解 |
| Baseline | 231.0 | +4.1% | 同路線，等距站牌 |
| Greedy | 220.0 | -0.9% | 同路線，貪心站牌略優於 MILP |
| GA (best) | 194.0 | **-12.6%** | 改變路線形狀，大幅優於 MILP |
| SA (best) | 205.0 | **-7.7%** | 改變路線形狀，優於 MILP |

> **關鍵洞察：** GA/SA 的最佳解優於 MILP，並非因為 MILP 求解失敗，而是因為 MILP 固定了 A* 最短路線。這證實了**路線形狀優化**（透過中繼點）對總成本的影響大於站牌位置優化。MILP 在本問題中不能作為全域 ground truth，而是「固定路線下的站牌最優基準」。
...
![Fig.5 — Map 2 路線形狀對比](assets/figures/fig05-route-comparison.png)

*Fig.5：Baseline 直線路徑（覆蓋率 13%）vs SA 偏移路線（覆蓋率 100%），路線形狀是成本差異的關鍵。*
...
![Fig.4 — 消融實驗雷達圖](assets/figures/fig04-ablation-radar.png)

*Fig.4：五組權重策略的三項成本雷達圖。不同策略在路線/步行/站牌成本間呈現清晰的取捨關係。*
